...
def messages():
    ...
    current_user.add_notification('unread_message_count', 0)  # 这里的unread_message_count 不是人的名字，是指消息个数的代称
    db.session.commit()
    page_start = request.args.get('page', 1, type=int)
    page_end = current_app.config['POSTS_PER_PAGE']
    messages = current_user.messages_received.order_by(Message.timestamp.desc()).paginate(page_start, page_end, False)
...
def notifications():
    ...
    json_text = jsonify([{'name': n.name, 'data': n.get_data(), 'timestamp': n.timestamp} for n in notifications])
    return json_text
