...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            flash('Invalid username or password')  # 会闪现一条消息到登录界面
    ...
    return render_template('login.html', title='Sign In', form=form)  # 此处不能使用 url_for()函数
...
def edit_profile():
    ...
    if form.validate_on_submit():  # 提交用户更新的数据
        ...
        flash('Your changes have been saved.')  # 提示
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Check your email for the instructions to reset your password')  # 给出提示
        return redirect(url_for('login'))
